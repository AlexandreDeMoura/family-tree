...
Capabilities are bounded user tasks with observable results. Keep their boundaries independent of implementation commits, reuse existing IDs for the same task, and keep review instructions, actual evidence, release membership, and delivery status honest. After changing code, update the affected manifest YAML files. Never create or edit `acceptance_history`, call the acceptance endpoint, accept or reopen a capability, or change delivery status on the builder's behalf; human review belongs to the builder's local Contour web action. Accept records review and sets `status: shipped` atomically; Reopen preserves delivery status.
